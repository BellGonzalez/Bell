...
-- 1- Cantidad de clientes y vendedores que tiene el negocio. --- Se realiza combinaciones de consultas con el operador UNION * Clase 9 *  
--- En cada resultado tiene que haber la misma cantidad de campos, los campos tienen que tener el mismo tipo de dato.

select 'clientes' descripción, count(*) cantidad from clientes
union 
...
union 
select v.nombre, v.apellido, v.email from vendedores v
...
-- 6- Listar los artículos cuyo nombre esté compuesto por más de 2 palabras. ---- Clase 5 
...
select fecha, count(*) as cantidad_de_facturas
from facturas
group by fecha
order by fecha desc;
...
select year(fecha) año, month(fecha) mes, sum(monto) as total_ventas
from facturas
...
select		c.id_cliente, c.nombre, c.apellido, count(*) 'cantidad de facturas'
from		clientes c
join		facturas f
...
on			c.id_cliente=f.id_cliente
join        ventas v
on          v.numero=f.numero and v.letra=f.letra
...
select		distinct(c.id_cliente), c.nombre, c.apellido, a.producto, v.cantidad
from 		clientes c
join		facturas f
on			c.id_cliente=f.id_cliente
join        ventas v
...
select    a.producto, sum(v.cantidad) as total_vendidos
from      articulos a 
left join ventas v
on	      a.codigo=v.codigo
group by  a.producto
...
-- 15- Listar el o los artículos que tengan la mayor cantidad de stock, siendo la mayor cantidad un único valor máximo. (clase 7 SUBQUERIES)

select a.codigo, a.producto, a.stock
from articulos a 
where stock=(select max(stock)from articulos);


-- 16- Agregar los campos precio_costo y precio_venta del tipo double a la tabla artículos. (Clase 06 agregar un campo) 

alter table articulos add  precio_costo double not null, add precio_venta double not null;  


-- 17- Agregar un campo del tipo decimal, llamado sueldo_empleado, a la tabla vendedores. EL mismo tendrá un valor inicial para todos de 450.010,00 (Clase01) DECIMAL(m,n)
-- en donde m es la cantidad de dígitos que tendrá el número en total y n es la cantidad de dígitos que se utilizarán para representar la parte decimal

alter table vendedores add sueldo_empleado decimal(10,2) default 450010.00;


-- 18- Actualizar todos los stock_minimo y stock_maximo a 100 y 500 respectivamente. (clase 05) "el editor de mysql Workbench trae configurado por defaul una protección para evitar ejecutar
-- comandos de borrado o actualización masiva. Hay que desactivarla desde el menú preferencias, otra manera de desactivarla es mediante código sql."
...
select * 
from articulos 
where codigo not in (select codigo from ventas);
...
select c.id_cliente, c.nombre, c.apellido, count(distinct a.producto) 'Articulos distintos comprados'
from   clientes c
join   facturas f
on     c.id_cliente=f.id_cliente
join   ventas v 
...
on     a.codigo=v.codigo
group by c.id_cliente
having count(distinct a.producto) > 5;
